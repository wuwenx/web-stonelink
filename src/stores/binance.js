...
export const useBinanceStore = defineStore('binance', {
  ...
  actions: {
    ...
    checkIfReconnectNeeded(oldConfig, newConfig) {
      const reconnectKeys = ['exchangeType', 'depthLevels'];
    ...
    handleBinanceMarkPriceData(data, symbol) {
      ...
      this.depthData[symbol].markPrice = processedData.markPrice;
      this.depthData[symbol].indexPrice = processedData.indexPrice;
      this.depthData[symbol].fundingRate = processedData.lastFundingRate;
      this.depthData[symbol].nextFundingTime = processedData.nextFundingTime;
      this.depthData[symbol].markPriceLastUpdate = new Date().toLocaleTimeString();
    },
