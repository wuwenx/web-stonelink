...
export default {
    ...
    data() {
        return {
            ...
            exchangeType: 'spot', // 'spot' 或 'futures'
            wsService: null,
            binanceStatus: 'disconnected',
            toobitStatus: 'disconnected',
            binanceLastUpdate: '--',
            toobitLastUpdate: '--',
            binanceAsks: [],
            binanceBids: [],
    ...
    computed: {
        priceDifference() {
            return this.binanceBestBid - this.toobitBestBid
        }
    },
    mounted() {
    ...
    methods: {
        ...
        // 连接Binance WebSocket
        connectBinance() {
            console.log('连接Binance WebSocket, 类型:', this.exchangeType)
            if (this.exchangeType === 'futures') {
                this.wsService.connectBinanceFutures(
                    this.selectedSymbol,
                    (data) => this.handleBinanceData(data),
                    (status) => {
        ...
        connectToobit() {
            this.wsService.connectToobit(
                this.selectedSymbol,
                (data) => this.handleToobitData(data),
                (status) => this.toobitStatus = status
            )
        },

        // 处理Binance数据
        handleBinanceData(data) {
            console.log('收到Binance数据:', data)
            if (data.e === 'depthUpdate') {
                ...
                this.binanceAsks.splice(0, this.binanceAsks.length, ...processedAsks)
                this.binanceBids.splice(0, this.binanceBids.length, ...processedBids)

                const bestPrices = DepthDataProcessor.calculateBestPrices(this.binanceBids, this.binanceAsks)
                this.binanceBestBid = bestPrices.bestBid
                this.binanceBestAsk = bestPrices.bestAsk

                this.binanceLastUpdate = new Date().toLocaleTimeString()
                console.log('Binance数据处理完成:', {
                    asksCount: this.binanceAsks.length,
                    bidsCount: this.binanceBids.length,
                    bestBid: this.binanceBestBid,
                    bestAsk: this.binanceBestAsk
                })

                // 强制触发Vue响应式更新
                this.$nextTick(() => {
                    console.log('Vue更新完成，当前数据:', {
                        binanceAsks: this.binanceAsks.length,
                        binanceBids: this.binanceBids.length
                    })
                })
            } else {
        ...
        handleToobitData(data) {
            ...
            this.toobitAsks.splice(0, this.toobitAsks.length, ...processedAsks)
            this.toobitBids.splice(0, this.toobitBids.length, ...processedBids)

            const bestPrices = DepthDataProcessor.calculateBestPrices(this.toobitBids, this.toobitAsks)
            this.toobitBestBid = bestPrices.bestBid
            this.toobitBestAsk = bestPrices.bestAsk

            this.toobitLastUpdate = new Date().toLocaleTimeString()
            console.log('Toobit数据处理完成:', {
                asksCount: this.toobitAsks.length,
                bidsCount: this.toobitBids.length,
                bestBid: this.toobitBestBid,
                bestAsk: this.toobitBestAsk
            })

            // 强制触发Vue响应式更新
            this.$nextTick(() => {
                console.log('Toobit Vue更新完成，当前数据:', {
                    toobitAsks: this.toobitAsks.length,
                    toobitBids: this.toobitBids.length
                })
            })
        },
